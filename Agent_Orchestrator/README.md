...
| `editor-save-all-command`              | Editor-agnostic shell command run before re-reading the prompt file on ENTER-twice (and at run start) to flush unsaved editor buffers to disk. Pass-through verbatim — no auto flag injection, so users pick their own editor recipe. Default `code --reuse-window --command workbench.action.files.saveAll` (VS Code). Recipes: Cursor `cursor --reuse-window --command workbench.action.files.saveAll`; VS Code Insiders `code-insiders --reuse-window --command workbench.action.files.saveAll`; Sublime `subl --command save_all`; JetBrains — no public save-all CLI, set `""`; vim — drive `--remote-send ':wa<CR>'` against a `--servername`; pure CLI (no editor) — set `""`. Deleting the key entirely falls back to the documented default (NOT silent no-op); set explicit `""` to disable. Legacy alias `vscode-save-all-command` is still honored. |
| `editor-save-flush-ms`                 | Milliseconds to wait after `editor-save-all-command` returns so the editor finishes writing to disk before the harness reads the file. Default `200`. Raise on slow disks. Legacy alias `vscode-save-flush-ms` is still honored. |
