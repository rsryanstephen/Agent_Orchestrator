...
**What it does.** One markdown history file per topic captures every prompt and every agent response (planning → coding → ask → assessment → fix). Agents are stateless `claude --print` calls; the harness reconstructs context from that file on each run and appends the new response.
...
hrun 5-all               # 3. run planning → coding → assessment → fix on topic 5
                         #    or: hrun 5-a to ask a question without code changes
```
...
# Shorthand: p|c|a|ac|f|af|pc|caf|all|pcaf|cont
...
hrun 5-a              # ask a question without code changes (topic 5)
hrun 5-ac             # ask then code: answer the question, then apply code changes
...
| `ask`        | Reads conversation context from the history file and answers the user's question. Does **not** modify any files. Appends `## Ask Agent Response`. Use `a` shorthand.                                                                                         |
...
| Command             | Pipeline                                                                                                             |
| ------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `ask-code`        | `ask` → `coding` (answer the question first, then apply code changes). Shorthand: `ac`.                        |
| `assess-fix`      | `assessment` → `fix` (assess the last coding work, then apply fixes). Shorthand: `af`.                         |
...
| **History file**            | A single markdown file per topic containing `## User Prompt` sections (you write these) interleaved with role-prefixed agent responses: `## Planning Agent Response`, `## Coding Agent Response`, `## Ask Agent Response` (when `ask` or `ask-code` ran), `## Assessment Agent Response`, and `## Coding Agent Response (Remediation)` (when a fix follows an assessment). A pipeline appends each phase's response in order and then a single trailing `## User Prompt` at the very end of the run. |
...
## Ask Agent Response            # only when ask or ask-code ran this turn
...
`hrun` launches any number of topic+command tokens concurrently and waits for all of them to finish. Token format: `<id>-<cmd>` (explicit topic id) or `<cmd>` (uses the last-touched topic). `<cmd>` is one of `p`, `c`, `a`, `ac`, `f`, `af`, `pc`, `caf`, `all`, `pcaf`, `cont`. `a` runs `ask` (answers question, no code changes); use `af` for assess+fix.
...
| `system-prompts.ask`                   | System prompt for the ask agent (answers questions, no file modifications). |
